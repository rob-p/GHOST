package javaasp;
import javaasp.RowDoneException;
import javaasp.BreakthruException;
import javaasp.DoneException;

public class LinearAssignmentSolver {

    public LinearAssignmentSolver() {}

    public void solve(int size, double[][] Array, int[] col_mate, int[] row_mate)
    {
	...
	parent_row= new int[n];for (i=0;i<n;++i) parent_row[i]=0;
	unchosen_row= new int[m]; for (i=0;i<m;++i) unchosen_row[i]=0;
	//col_mate= new int[m];
	for (i=0;i<m;++i) col_mate[i]=0;
	//row_mate= new int[n];
	for (i=0;i<n;++i) row_mate[i]=0;
	row_dec= new double[m]; for (i=0;i<m;++i) row_dec[i]=0;
	col_inc= new double[n]; for (i=0;i<n;++i) col_inc[i]=0;
	slack= new double[n];  for (i=0;i<n;++i) slack[i]=0;
	slack_row= new int[n]; for (i=0;i<n;++i) slack_row[i]=0;
	...
	try {
		if(t==0) throw new DoneException(); //goto done;
		unmatched= t;
		while(true){
		    q= 0;
		    try {
			...
			while(q<t){
				...
				for(l= 0;l<n;l++)
					...
					if(del<slack[l]){
					    if(del==0){
						if(row_mate[l]<0) throw new BreakthruException();
						slack[l]= 0;
						parent_row[l]= k;
		...
		if(--unmatched==0) throw new DoneException();
		t= 0;
		for(l= 0;l<n;l++){
		    parent_row[l]= -1;
		    slack[l]= 10000000;
